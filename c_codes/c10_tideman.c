...
int main(int argc, string argv[])
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        ...
        {
            locked[i][j] = false;
			preferences[i][j] = 0;	// Initialize preferences matrix
        }
    }
...
void add_pairs(void)
{
    // Loop through preferences array and check all possible pairs
	for (int i = 0; i < candidate_count; i++)
	{
		for (int j = i + 1; j < candidate_count; j++)
		{
			...
			{
				// If candidate [j] is preferred over candidate [i]
...
// Test for cycle by checking arow coming into each candidate
bool cycle(int end, int cycle_start)
{
